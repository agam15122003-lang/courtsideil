...
4. `supabase_migrations_ledger.sql` — טבלת `schema_migrations` + `mark_migration()`. **(הורץ 24.7)**
5. `supabase_security3.sql` — נעילת `role` אחרי הרשמה, קהילת השחקנים רק לשחקן שאושר, `resolve_join_code()`. **(הורץ 24.7)**
6. `supabase_privacy4.sql` — **קריטי:** מסיר את הרשאת הקריאה הרוחבית מ-`profiles` ומעניק רשימת עמודות
...
> ורק אז להריץ SQL שמצמצם הרשאות.** בדיקה: `select * from public.schema_migrations order by ran_at;`
> וגם `select has_column_privilege('authenticated','public.profiles','phone','select');` (חייב `false`).
...
## לראות את «תיק שחקן» בלי חשבון (22.8.2026)
`node tools/dossier-harness/shoot.mjs` — בונה את המסך האמיתי עם ה-CSS האמיתי ו-`dossierApi` מוחלף
בנתוני דמה (vite plugin), ומצלם מחשב/אייפד/טלפון + חמשת הטאבים ל-`tools/dossier-harness/dist/`.
זה מה שתפס את מה שהבנייה לא תופסת: שם שנעלם על רקע כהה, תוויות SVG שנחתכו, כותרת שנערמה לעמודה.
הדפוס ניתן להעתקה לכל מסך שמדבר עם המסד דרך מודול API אחד.

## אייפד (19.8.2026) — שלושה מצבים, כולם מותנים ב-`pointer: coarse`
- **769–840 (אייפד עומד):** שלד המגירה של הטלפון תמיד — טופ-בר + המבורגר, כל הרוחב לתוכן. (עד אז: סרגל 232px ותוכן מעוך.)
- **841–1400 (אייפד שוכב):** הסרגל נשאר כמו במחשב, **אלא** כשפתוחים המחברת (`.nbk-focus`) או דף התרגיל (`.drillform-head`) — אז הבאנר/הציטוט/הטאבים מתקפלים והסרגל הופך למגירה. סוגרים את העורך — הכול חוזר.
- **700–768 (מיני עומד):** שלד הטלפון כרגיל; בעריכה הבאנר והגלולה הצפה מתקפלים.
- **עט מול אצבע:** `ink.jsx` מדליק `html.has-pen` ל-8 שניות מכל נגיעת עט; תחת המחלקה הזו האצבע גוללת (`touch-action: pan-y`) והעט מצייר. בלי עט — אצבע מציירת כרגיל.
- **לבדוק:** Playwright עם `hasTouch: true` ברוחבים 834/1024 (בלי אמולציית מגע שום דבר מזה לא נדלק — `pointer: coarse`), ומחשב-עם-עכבר וטלפון 390 חייבים להישאר זהים. ה-harness של 19.8 יושב ב-scratchpad של אותו סשן; הדפוס: דף סטטי עם שלד ה-DOM + ה-CSS מה-dist.
...
## סבב עיצוב · נגישות · פרטיות — 24–25.7.2026 (מוזג ל-main מענף `design-ux-pass-jul24`)
סקירה חיה של האפליקציה המחוברת (מאמן ושחקן, 1920 ו-384px אמיתי) + ביקורת קוד מרובת-סוכנים.

**באגים שהפילו מסכים:** `src/SendToPlayers.jsx` ו-`src/sendToPlayers.js` נבדלו רק באות גדולה —
ב-Windows Vite פתר את הייבוא חסר-הסיומת לקובץ העזר (שאין בו default export) והאפליקציה כולה
נמחקה למסך לבן במסך הקבוצות, בזמן שבלינוקס/Vercel הכול עבד. העזר שונה ל-`sendToPlayersApi.js`,
ונוסף `scripts/check-imports.mjs` + `npm run check` + שלב ב-CI שנכשלים על ייבוא עמום והתנגשות
שמות. נוסף `src/ErrorBoundary.jsx` סביב שני הדשבורדים (קריסה במסך אחד מחקה קודם את כל האפליקציה)
שמדווח ל-`client_errors` עם שם המסך (`coach:teams` / `player:goals`).

**נגישות שנמדדה, לא שהוערכה:** `--text-muted` הוכהה ל-`#55647A` (נמדד 4.33:1 על הקנבס הקרם);
רצפת 12px ל-16 סלקטורים; **שם המאמן בסייד-בר היה שחור על נייבי — 1.03:1**; `.video-card` של דף
הבית חטף את מסך המדיה (כותרת כהה על נייבי + כוכב חמישי חתוך שמנע דירוג 5); צ׳יפ פעיל בעולם
השחקן היה 1.98:1 במצב כהה (עכשיו 10.53:1); `useFocusTrap.js` ב-9 דיאלוגים; מאזין `Enter` גלובלי
ב-`confirm.jsx` שאישר מחיקה מכל מקום במסך — הוסר; המגירה מקבלת `inert` כשסגורה; H1 אחד לכל מסך.
**בסוף הסבב: 0 כשלי ניגודיות, 0 טקסט מתחת ל-12px, אין גלישה אופקית ב-384px, אין שגיאות קונסולה.**

**מספרים שקריים שתוקנו:** הקהילה הכריזה "6 מאמנים" (ספרה את כל `profiles`, כולל שחקנים) → נספר
`coach_directory`; לוח הניהול ספר חשבון שחקן בתוך "מאמנים" והציג אותו עם כפתורי אימות/חסימה.

**אל תתקנו שוב — נבדק ונמצא תקין:** יעדי המגע בפוטר/ביומן/בסימנייה (מכוסים ב-`@media (pointer: coarse)`
שלא מופעל בדפדפן שולחני); `card-enter` לא "מעלים תוכן לתמיד"; גוטר השעות בלו"ז כבר `sticky`;
ריבועי הפודקאסט הירוקים הם ירוק-ספוטיפיי מכוון. **איחוד ה-CSS לא שווה את הסיכון** — הרווח הנמדד
אחרי gzip הוא 2–3.5KB בלבד.

## פתוח / רעיונות להמשך
- **מסך "הקבוצות שלי" עמוס ולא מובן, במיוחד אזור המטרות ובמיוחד במובייל** (הבקשה המפורשת של הבעלים,
  25.7). נמדד ב-384px: טאב "סגל" בגובה 1,838px עם 24 פקדים (כי `.team-split` מתקפל ומטיל את
  "המשחק הבא" ו"טבלת הליגה" מתחת לסגל — כפילות של טאבים שקיימים), וטאב "מטרות" עם 22 פקדים המחזיק
  **שני מודלים לא-קשורים**: `TeamGoalsBoard` (מובנה, משותף עם השחקנים) מעל שלושה כרטיסי טקסט חופשי
  כמעט זהים לשבוע/חודש/עונה (`team_goals`, למאמן בלבד, כל אחד עם בורר תקופה וכפתור שמירה נפרד).
  פאנל עיצוב (3 הצעות → 3 שופטים → סינתזה) הופעל; העיצוב טרם יושם.
- להתאים את **צד המאמן** יותר לרוח העיצוב החדש (Teams/tabs, home card) בלי לשבור פיצ׳רים —
  הפריטים המדידים: רדיוס כרטיסים 10–12px מול 20px בעולם השחקן, ופס ההירו עם פינות תחתונות מרובעות.
- אולי גרף התקדמות גם בצד המאמן (לראות התקדמות שחקן במטרה).
- **חוסר-כיסוי מוכר:** גיליונות השחקן (`FeedbackSheet`, `PlayerGoals` AddGoalSheet) עוד בלי מלכודת
  פוקוס, ו-`client_errors` ואישור הסרטונים עוד בלי UI בלוח הניהול (הסרטונים הקיימים אושרו
  אוטומטית, ולכן רק ייבוא חדש נכנס כלא-מאושר ואין דרך למצוא אותו).
- אחדות ניסוח בעברית: המניין הוא ~5:1 לטובת גוף שני יחיד ("אתה"); יש ~30 מחרוזות ב"אתם", כולל
  שבע שמתנגשות בתוך אותו משפט. ההכרעה על הטון עוד לא בוצעה.
