...
// ===== וידאו של האתגר =====
// מסלול נפרד לגמרי מהתמונות, ובכוונה. שלושה הבדלים שכל אחד מהם הוא
// החלטת בטיחות ולא סגנון:
//
//  1. **בלי נפילה לכתובת ציבורית.** signedMediaUrl נופלת ל-getPublicUrl
//     כשהחתימה נכשלת — התנהגות נכונה לתמונת תרגיל, והרסנית לקליפ של
//     קטין: כשל הרשאה היה הופך לכתובת ציבורית עובדת. דליפה שמתחפשת
//     להצלחה. כאן כשל מחזיר null, והנגן פשוט לא מוצג.
//  2. **בלי קאש.** הנתיב דטרמיניסטי (challenges/<uid>/<challengeId>),
//     ולכן אחרי החלפת טייק קישור מהקאש היה מגיש את הגרסה הישנה —
//     כלומר האדמין היה מאשר סרטון אחר מזה שהשחקן הגיש.
