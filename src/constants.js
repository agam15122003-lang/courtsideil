...
// חייבת להיות זהה למזהה הגרסה בראש privacy.html ו-terms.html, וגם לשורות
// 'terms' ו-'privacy' ב-consent_documents — אחרת אי אפשר לקשור רשומת
// הסכמה לנוסח שאושר. accessibility.html מנוהל בגרסה נפרדת: תוכנו לא
// השתנה ב-v3 והוא נשאר v2 · 24.8.2026.
//
// ⚠ מקום רביעי שאינו כאן: **אפליקציית האנדרואיד**. היא אורזת עותק משלה
// של privacy.html / terms.html ואת המחרוזת הזו מהודרת בתוך ה-APK. אחרי
// שינוי כאן חובה `npm run build && npx cap sync android`, לבנות APK חדש
// ולהפיץ אותו — עד אז מכשירים מציגים את הדפים הישנים ורושמים בהרשמה את
// גרסת התנאים הישנה. נכון ל-6.9.2026 העותק הארוז הוא עדיין v2 · 24.8.2026.
...
// 4.9.2026 — v3: צד השחקן חזר (פיילוט), ולכן משפטי «אין חשבונות שחקן /
// אין קטינים / אין מנגנון הסכמת הורים» הפכו שקריים ותוקנו. נוסף תיאור
// מלא של הצ'ק-אין היומי (שינה/אנרגיה/גוף/חולה): מטרה, מי רואה, 90 יום,
// כיבוי ומחיקה. ⚠ אותה מחרוזת בדיוק חייבת לשבת גם ב-consent_documents
// עבור 'terms' ו-'privacy' — ראו supabase_legal_v3_4_9.sql.
// 6.9.2026 — v3 · 6.9.2026: המסמכים תוקנו מהותית לפני הפרסום (ביטול «אני
// חולה היום» מהמסך, ותשובות הצ'ק-אין שנכנסו לייצוא «המידע שלי»), ולכן
// המחרוזת נושאת את התאריך החדש. המסמכים טרם פורסמו, ואף הסכמה לא נחתמה
// על v3 · 4.9.2026 — הנוסח ההוא לא פורסם.
export const TERMS_VERSION = 'v3 · 6.9.2026'
