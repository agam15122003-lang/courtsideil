...
export default function Community({ session, profile }) {
  ...
  async function load(opts = {}) {
    ...
    const { data, error } = await supabase
      ...
      .select(
        '*, author:profiles(first_name, last_name, club, avatar_url), likes:community_post_likes(user_id), comments:community_post_comments(id, user_id, content, created_at, author:profiles(first_name, last_name, avatar_url))'
      )
  ...
  // ---------- מסך התקנה — הטבלאות עוד לא קיימות במסד ----------
  if (sqlMissing) {
    return (
      <div className="welcome-card">
        ...
        <div className="empty-state">
          ...
          <div className="empty-title">{L('נשאר צעד אחד להפעלת הקהילה', 'One step left to enable the community')}</div>
          <p className="muted small" style={{ maxWidth: 480 }}>
            {L(
              'צריך ליצור את טבלאות הקהילה במסד הנתונים (פעם אחת): פתח את Supabase → SQL Editor, הדבק את התוכן של הקובץ supabase_community.sql מהפרויקט ולחץ Run. אחרי זה רענן את העמוד.',
              'The community tables need to be created once: open Supabase → SQL Editor, paste the contents of supabase_community.sql from the project and click Run. Then refresh this page.'
            )}
          </p>
          <button type="button" className="btn-primary empty-cta" onClick={() => load()}>
      ...
      <header className="page-header">
        <div className="page-header-text">
          ...
          <p className="page-desc">
            {L(
              'המגרש המרכזי של כולנו — שיתופים, שאלות וצילומים מהאימונים.',
              "Everyone's home court — posts, questions and photos from practice."
            )}
          </p>
        </div>
      </header>
