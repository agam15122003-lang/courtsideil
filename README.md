...
| 46 | `supabase_checkins_4_9.sql` | **צ'ק-אין בוקר (פיילוט, ראו `הרצת_SQL_4.9.md`)** — טבלה `player_checkins` (שינה/אנרגיה/גוף/חולה, בלי טקסט חופשי) + RLS (שחקן רק את שלו ורק בסגל; מאמן קורא את קבוצתו) + שער הסכמה RESTRICTIVE, עמודות `team_players.wellness_off` («בלי שאלות», כיבוי לילד) ו-`team_iba.coach_role` («עוזר מאמן», תצוגה), פונקציות `ack_checkin/ack_checkins/handle_checkin/delete_my_checkins`, ושמירה 90 יום (`checkins_purge`, יבש כברירת מחדל). אחרי #44+#45. האפליקציה עובדת גם בלעדיו — הכרטיס והרצועה פשוט לא מוצגים |
| 47 | `supabase_legal_v3_4_9.sql` | **הנוסחים המשפטיים v3 (ראו `הרצת_SQL_4.9_משפטי.md`)** — `update` בלבד על `public.consent_documents`: `terms`+`privacy` → `v3 · 6.9.2026` (מיישר את המסד עם `public/*.html` ועם `TERMS_VERSION`; הם היו תקועים על `v1 · 3.8.2026`), ו-`doc_a` → `v2 · 6.9.2026` עם נוסח חדש שכולל את הצ'ק-אין היומי. `on conflict do **update**` (הזריעה המקורית הייתה `do nothing` — דפוס שלא היה משנה כלום). לא נוגע ב-`consents`, ב-`approval_status` ובבקשות הסכמה פתוחות. ⚠ תופעות לוואי מכוונות: באנר «לאשר מחדש» אצל קטינים קיימים, `create_consent_request` מפסיקה להחזיר `already_consented`, ו-`game_publish_ok` מחזירה false (לא מזיק — «עולם הכדורסל» מוסתר). האפליקציה עובדת גם בלעדיו, אבל אז המסד חותם על הסכמות גרסה שאינה הנוסח שהוצג |
| 48 | `supabase_pilot_fixes_6_9.sql` | **תיקוני הפיילוט (ראו `הרצת_SQL_6.9.md`)** — מחליף ארבע פונקציות קיימות ומהדק הרשאות, בלי טבלאות חדשות ובלי מחיקת מידע: (1) `join_with_code` — בקשת הצטרפות שנדחתה נפתחת מחדש ל-`pending` בניסיון חדש עם קוד תקין (עד היום דחייה, גם בטעות מגע, נעלה את הילד לתמיד בזמן שהמסך אמר לו «הבקשה נשלחה»); מחזירה `reopened:true` בלי `already`, כדי שהלקוח ישלח למאמן התראה. (2) `_data_export_for` — «המידע שלי» כולל מעכשיו מדור `checkins` (+`checkins_coach_logged`) ומזכיר אותו בטקסט ההסבר; נשארת SECURITY INVOKER. (3) `player_checkins` — `revoke delete` מ-authenticated ופיצול `pc_player_own` ל-`pc_player_read/insert/update` (אותם תנאים בדיוק), כדי שדגל אדום לא יימחק בשקט; `delete_my_checkins` נשארת ורושמת שורת יומן **מסכמת** (כמות/טווח/כמה מסומנים, בלי התשובות). (4) `purge_checkins_for_roster(uuid)` — מחיקת כל דיווחי ילד אחד כשההורה מבקש (אין כפתור במסך; ידני מה-SQL Editor). (5) `retention_run_scheduled` — הריצה המתוזמנת מנקה גם את הצ׳ק-אין (עדיין **יבשה** כל עוד `armed=false`). אחרי #46. ⚠ אם מריצים שוב את #46 / `supabase_hardening_medium_3_8.sql` / `supabase_my_data.sql` / `supabase_retention_schedule.sql` — להריץ את הקובץ הזה שוב אחריהם. האפליקציה עובדת גם בלעדיו |
| 49 | `supabase_checkin_flags_12_9.sql` | **צ'ק-אין — הדגל שנסגר חוזר (ראו `הרצת_SQL_12.9.md`)** — מחליף שתי פונקציות של #46, בלי טבלאות חדשות ובלי שינוי סכימה: (1) `checkins_touch` — שינוי אמיתי באחת מעמודות התשובה (`sleep_bucket/energy/body/pain_area/pain_blocks/sick`) מאפס `coach_ack_at/by` ו-`handled_at/by`, כך ש«דיברתי איתו» של הבוקר לא בולע את הדיווח הבא של אותו יום (יש שורה אחת לילד ליום) ו«ראיתי» לא נשאר על תוכן שהשתנה; עדכון שכותב אותו ערך אינו מאפס, ופונקציות המאמן (`ack_checkin`/`handle_checkin`) לא מאפסות את עצמן. (2) `ack_checkins` — מסמן רק שורות שיש בהן תשובה (`sleep_bucket`/`energy`/`body`), זהה ל-`hasAnswer` ב-`NextPractice.jsx`: שורה ריקה (מה שנשאר אחרי «בעצם אני בסדר») אינה «דיווח», ועד היום השרת סימן שורות שהמסך אינו סופר והמאמן קיבל «לא היה מה לסמן». (3) `revoke delete on player_checkins from authenticated, anon` — חוזר בכוונה על #48 סעיף 3א, כדי שפרצת מחיקת הדגל תיסגר גם אם רק הקובץ הזה ירוץ; שני הקבצים בטוחים בכל סדר. אחרי #46. האפליקציה עובדת גם בלעדיו |
