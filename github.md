...
| ענף עבודה | `redesign` — **נדחף** ל-GitHub |
| ענף פרודקשן | `main` — עומד על `58474c7` (27.7). **`redesign` עדיין לא מוזג אליו, ולכן האתר החי לא כולל את עבודת 28–29.7.** |
...
מעמוד 22 במסמך: סרגל תחתון של 5 יעדים בשני העולמות, שלדי טעינה בכל רשימה, שתי דלתות בלבד לבניית תוכנית, ו-eyebrow אחיד בדף הבית. «מיזוג סרטונים לתרגילים» מתנגש בהנחיה קודמת של הבעלים («המדיה נשארת מסך נפרד») — דורש הכרעה שלו.
